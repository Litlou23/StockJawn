...
export async function GET() {
  ...
  try {
    const res = await fetch(`${base}/api/paper-stock-candidates/stats`, { cache: 'no-store' });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch {
    return NextResponse.json({ error: 'Failed to fetch stock learning stats' }, { status: 500 });
  } finally {
