...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const url = id ? `${base}/api/portfolio/dashboard/${id}` : `${base}/api/portfolio/dashboard`;

    const res = await fetch(url, { cache: 'no-store' });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch {
    return NextResponse.json({ error: 'Failed to fetch portfolio dashboard' }, { status: 500 });
  } finally {
