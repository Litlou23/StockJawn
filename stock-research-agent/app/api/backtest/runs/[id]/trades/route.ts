...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${base}/api/backtest/runs/${encodeURIComponent(id)}/trades`, { cache: 'no-store' });
    const data = await res.json();
    return NextResponse.json(data, { status: res.status });
  } catch (err) {
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to fetch run trades' },
      { status: 502 },
    );
  } finally {
