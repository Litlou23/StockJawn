...
/** Fire-and-forget proxy for POST /api/backtest/sweep. */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (err instanceof DOMException && err.name === 'TimeoutError') {
      return NextResponse.json({
        status: 'started',
        message: 'Parameter sweep started. Poll /api/backtest/sweep-status for progress.',
      });
    }
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to start sweep' },
      { status: 502 },
    );
  } finally {
