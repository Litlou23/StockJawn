...
/** Fire-and-forget proxy for POST /api/meta-labeler/label. */
export async function POST(req: NextRequest) {
  ...
  try {
    const res = await fetch(`${base}/api/meta-labeler/label?limit=${encodeURIComponent(limit)}`, {
      method: 'POST',
    ...
    if (err instanceof DOMException && err.name === 'TimeoutError') {
      return NextResponse.json({
        status: 'started',
        message: 'Labeling started. Poll /api/meta-labeler/status for progress.',
      });
    }
    return NextResponse.json(
      { error: err instanceof Error ? err.message : 'Failed to start labeling' },
      { status: 502 },
    );
  } finally {
