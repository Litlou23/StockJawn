...
- `NeutralOutcomeEvaluator` evaluates open neutral predictions (neutral_high_volatility, neutral_no_edge, neutral_range_bound) into `neutral_prediction_outcomes`. Each type has its own accuracy logic: high_volatility checks realized vol and directional clarity, no_edge checks for breakouts and directional persistence, range_bound checks support/resistance adherence. All types include a counterfactual analysis using the original bull/bear scores to measure whether a directional call would have been correct and how significant the missed opportunity was.
...
**`neutral_prediction_outcomes`** — outcome evaluation for non-directional predictions (neutral_high_volatility, neutral_no_edge, neutral_range_bound). Unlike `paper_stock_outcomes` which grades directional predictions as correct/incorrect, this table measures whether the neutral classification was justified. Core metrics: `realized_move_percent`, `absolute_move_percent`, `max_run_up`/`max_drawdown`, `realized_volatility`. Type-specific: `volatility_prediction_accuracy` (high_volatility), `range_adherence_percent`/`support_broken`/`resistance_broken` (range_bound), `breakout_occurred`/`directional_persistence` (no_edge). Counterfactual analysis: `counterfactual_direction`/`counterfactual_correct`/`opportunity_missed_score` with original bull/bear scores — answers "what if the system had made a directional call?" `neutral_accuracy_score` (0-100) summarizes overall classification quality. Evaluated by `NeutralOutcomeEvaluator` during EOD review (Step 6 in `DynamicPickOrchestrator`). Used by: future Learning improvements.
