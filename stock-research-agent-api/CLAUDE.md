...
**expected_value_percent**: `EV = (winProb × gain%) - (lossProb × loss%)`. Positive EV = trade worth taking over many repetitions. Computed from confidence (as win probability), target price (gain), and stop price (loss). Directional rankings use EV as primary sort key instead of confidence alone.
